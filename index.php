...
try {
    ...
    $http->on("request", function ($request, $response) use ($http) {
        ...
        if (!isset($request->header['host'])){
            $request->header['host'] = "yyshou.com";
        }

        //設置域名
        if (preg_match('/\w*\.\w*$/', $request->header['host'], $_hosts)) {

            (defined('Dom') && !empty(Dom)) ? "" : define('Dom', $_hosts[0]);
        } else {
            (defined('Dom') && !empty(Dom)) ? "" : define('Dom', "yyshou.com");
        }
